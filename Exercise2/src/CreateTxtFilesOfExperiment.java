...
public class CreateTxtFilesOfExperiment{
    ...
    //@param j start from 0 to input user's number of folders,become 0 every time a file is created
    ...
    public void createTxtFiles(StringBuilder path)
    {
        ...
        while(exit != 0)
        {
            ...
            //user give number of Folders,and read csv file-check if user have given 3 different numbers and maxDifference
            while(true)
            {
                ...
                try
                {
                    ...
                    {
                        if (countN >= 1){
                            csvReader.readCsvFile(path, n, secondFlag);
                            secondFlag = false;
                        }
            ...
            while(i < numberOfFiles && csvReader.getExist())
            {
                ...
                //delete the value of String builder because should be empty for the next file
                ret.delete(0,  ret.length());
                
                //run greedy algorithm without sort for a file of specific numbers of Folders
                greedy.greedyAlg(currentFileList.getHead());
                numberOfDisksAlg1 = numberOfDisksAlg1 + greedy.getNumberOfDisks();

                //run greedy algorithm with sort for a file of specific numbers of Folders
            ...
            i = 0;
            //create csv file,send flag to createCsvFile method so program know that header line and csv file have been created
            if(csvReader.getExist()){
